# Week 07 — 바이브코딩 쇼케이스 + 2인 개인지도 워크숍

> **수강생 2명 맞춤**: 김인혜 과장님(영업관리/이상출고 모니터링) · 육아름 차장님(해외관리/Netra 대시보드)
> **전반 60분**: 쇼케이스 5종 — 두 분 업무와 **각각 매핑**되는 데모
> **후반 30분**: 두 분 개인 지도 (15분씩)
...
│ D. PPT·HTML·JSX 3종 (15분)    — 육아름님 Netra 직결 ⭐  │
│ F1. 메일 수신 취합 (12분)     — 육아름님 업무 직결 ⭐   │
│ F2. 이상거래 알림 (12분)      — 김인혜님 업무 직결 ⭐   │
└─────────────────────────────────────────────────────┘
        ▼
┌─ 65~75분 쉬는 시간·질문 ─┐
└────────────────────────┘
        ▼
┌─ 75~105분 2인 개인지도 (30분, 각 15분) ───┐
...
| 66~75분 | 쉬는 시간 + 자유 질문 | — |
| 75~90분 | **김인혜님 개인 지도** (본인 업무 + F2 응용) | [수강생과제_CLAUDE_템플릿.md](수강생과제_CLAUDE_템플릿.md) |
| 90~105분 | **육아름님 개인 지도** (본인 업무 + D/F1 응용) | 위와 동일 |
| 105~120분 | 마무리 + 8주차 예고 | — |
...
**두 분에게**: "핸드폰으로 찍기만 하면 경비 정리 끝. 매달 얼마나 걸리세요?"
...
**두 분에게**: "김상헌 과장님 환율 리포트와 원리 같죠? 외부에서 가져와 → AI 요약 → 보고서"

---

### 데모 D — 3종 대시보드 (15분) [상세](demos/dashboard_3format/README.md) ⭐ 육아름님 직결
...
**육아름님에게**: "지금 Streamlit으로 만드시는 Netra 대시보드가 바로 이 구조. 같은 데이터로 경영회의용 PPT, 내부 포털 HTML, Netra 이식용 React까지 한 번에 뽑을 수 있어요"
...
### 데모 F2 — 이상거래 자동 알림 (12분) [상세](demos/email_send/README.md) ⭐ 김인혜님 직결
...
**김인혜님에게**: "지금 하시는 이상출고 감지 + 담당자 메일 알림이 완벽히 이것. Windows 스케줄러에 등록하면 매주 자동"

---

## 5개 데모 뒤 핵심 질문

각 데모 끝에 두 분께 던질 질문 (2명이라 개인 답 가능):
...
| 수강생 | 추천 접목 방향 |
|---|---|
| **김인혜 과장님** | F2 (이상거래 알림) 변형 — 본인의 SAP 기준으로 이상거래 추출 + 실제 Outlook 초안 작성 |
| **육아름 차장님** | D (대시보드 3종) 변형 — 지금의 Streamlit을 PPT/React로도 변환 / 또는 F1 메일 취합 |

다른 분이 옆에서 관찰 → 15분 뒤 교체. 관찰하는 시간도 학습.
