"""
파이프라인 오케스트레이터: fetch_rates + import_sales를 함수 import로 묶기.
...
def main():
    ...
    with open(log_path, "a", encoding="utf-8") as fp:
        ...
        # Step B: 매출 import (환율 실패해도 진행 — 두 단계는 독립)
        log_line(fp, "[매출 import] 시작")
        step_start = time.time()
        try:
            result = import_sales.main()
            log_line(
                fp,
